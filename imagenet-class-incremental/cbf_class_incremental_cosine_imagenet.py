...
from utils_incremental.compute_accuracy_TDE import compute_accuracy_TDE, get_cos_sin
    ...
    for iteration in range(start_iter, int(args.num_classes/args.nb_cl)):
        ...
        for iteration2 in range(iteration+1):
            for iter_dico in range(args.nb_cl):
                ...
                assert((alph_icarl[num_samples:]==0).all())
                alph_icarl = alph_icarl[:num_samples]
                ...
                alph_NCM = np.ones(num_samples)/num_samples
        ...
        if iteration > start_iter and args.cb_finetune:
            ...
            if args.ft_flag == 0:  # everything is not updated 
                ...
                if args.TDE:
                    ...
                    print("incremental_train_and_eval_MR_LF_TDE")
                    tg_model, causal_embed = incremental_train_and_eval_MR_LF_TDE(args.ft_epochs, tg_model, ref_model, tg_ft_optimizer, tg_ft_lr_scheduler, \
                        trainloader, testloader, \
                        iteration, start_iter, \
                        cur_lamda, \
                        args.dist, args.K, args.lw_mr, \
                        causal_embed=causal_embed, fix_bn=fix_bn_flag)
                    np.save(ckp_path+'/CBF_causal_embed_run_{}_iteration_{}.npy'.format(iteration_total, iteration), causal_embed) 
