...
all_accu_matrix = torch.zeros([all_step_num, all_step_num+1, 3])
# top 1 accuracy CNN
...
for iteration in range(start_iter, int(100/nb_cl)):
    ...
    evalloader = torch.utils.data.DataLoader(evalset, batch_size=128,
        shuffle=False, num_workers=2)
    acc = compute_accuracy(tg_model, tg_feature_model, current_means, evalloader, print_info=False)
    all_accu_matrix[step_i, all_step_num] = torch.tensor(acc)
    step_i += 1
...
accu_cumul_mean_CNN = torch.mean(all_accu_matrix[:, -1, 0])
accu_cumul_iCaRL = all_accu_matrix[:, -1, 1]
accu_cumul_mean_iCaRL = torch.mean(all_accu_matrix[:, -1, 1])
accu_cumul_NCM = all_accu_matrix[:, -1, 2]
accu_cumul_mean_NCM = torch.mean(all_accu_matrix[:, -1, 2])
